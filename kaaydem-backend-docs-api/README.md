...
Le détail complet des endpoints (méthodes, autorisations, paramètres, codes d'erreur) se trouve dans
[`docs/documentation-api-kaaydem.md`](docs/documentation-api-kaaydem.md).

Une collection Postman prête à l'emploi est également disponible dans
[`docs/kaaydem-api.postman_collection.json`](docs/kaaydem-api.postman_collection.json) — importez-la dans
Postman, connectez-vous via la requête "Connexion" (le token est enregistré automatiquement), puis testez
n'importe quel endpoint protégé sans configuration supplémentaire.
